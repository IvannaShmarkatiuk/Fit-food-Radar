...
export function RestaurantCard({ restaurant, onClick, isFavorite, onToggleFavorite }: RestaurantCardProps) {
  ...
  return (
    <div onClick={onClick} className="bg-[#1A1A1A] border border-[#333333] rounded-2xl overflow-hidden cursor-pointer group hover:border-[#4A4A4A] transition-all flex flex-col h-full relative">
      ...
      <button onClick={(e) => { e.stopPropagation(); onToggleFavorite?.(e); }} className="absolute top-4 right-4 p-2 bg-black/40 backdrop-blur-md rounded-full border border-white/10">
      ...
      <div className="p-5 flex flex-col flex-grow">
        <div className="flex justify-between items-start mb-2">
          ...
          <div className="flex items-center gap-1 bg-[#252525] px-2 py-1 rounded-md">
            <StarIcon size={14} className="text-[#C45A2A] fill-[#C45A2A]" />
        ...
        <div className="flex flex-wrap gap-1.5 mt-auto">
          {((restaurant as any).categories || (restaurant as any).Categories)?.map((c: any) => (
            <span key={c.id || c} className="text-[10px] uppercase font-bold text-[#C45A2A] bg-[#C45A2A]/10 px-2 py-1 rounded">
              {c.name || c.Name || c}
            </span>
          ))}
        </div>
