...
export function RestaurantCard({ restaurant, onClick, isFavorite, onToggleFavorite }: RestaurantCardProps) {
  ...
  // Додаємо / на початок, якщо Маша вказала шлях без нього (наприклад "images/bufet.jpg")
      ...
      <div className="p-5 flex flex-col flex-grow">
        ...
        <div className="flex flex-wrap gap-1 mt-auto">
          {((restaurant as any).categories || (restaurant as any).Categories)?.map((c: any) => (
            <span key={c.id || c} className="text-[10px] uppercase font-bold text-[#C45A2A] bg-[#C45A2A]/10 px-2 py-1 rounded border border-[#C45A2A]/20">
              {c.name || c.Name || c}
            </span>
          ))}
        </div>
