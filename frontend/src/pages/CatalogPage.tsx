...
export const CatalogPage: React.FC<CatalogPageProps> = ({ 
  ...
  const categories = useMemo(() => {
    // Просто збираємо всі рядки з масивів категорій кожного ресторану
    const all = restaurants.flatMap(r => (r as any).categories || (r as any).Categories || []);
  ...
  const filtered = useMemo(() => {
    return restaurants.filter(r => {
      ...
      const matchesCat = activeCategory === 'Всі заклади' || cats.includes(activeCategory);
      return matchesSearch && matchesCat;
    });
  }, [restaurants, searchQuery, activeCategory]);

  if (isLoading) return <div className="text-center py-20 text-[#EDE8D0] animate-pulse font-medium">Шукаємо смачні заклади...</div>;
  ...
  return (
    <main className="max-w-[1440px] mx-auto px-8 py-12 flex flex-col lg:flex-row gap-12">
      <aside className="w-full lg:w-64 flex-shrink-0">
        <div className="sticky top-28">
          <div className="flex items-center gap-2 mb-6 text-[#C45A2A]">
            <FilterIcon size={18} /> <h3 className="text-[#EDE8D0] font-bold uppercase text-[10px] tracking-widest">Категорії</h3>
          </div>
      ...
      <div className="flex-grow grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8 items-start">
        {filtered.map(r => (
