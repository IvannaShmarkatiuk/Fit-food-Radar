      ...
      <aside className="w-full lg:w-64 flex-shrink-0">
        <div className="sticky top-28">
          <div className="flex items-center gap-2 mb-6 text-[#C45A2A]">
            <FilterIcon size={18} /> <h3 className="text-[#EDE8D0] font-bold uppercase text-xs tracking-widest">Фільтри</h3>
          </div>
          <div className="flex flex-wrap lg:flex-col gap-1">
            {categories.map(cat => (
              <button key={cat} onClick={() => setActiveCategory(cat)} 
                className={`px-4 py-2.5 rounded-xl text-left text-sm font-medium transition-all ${activeCategory === cat ? 'bg-[#C45A2A] text-white' : 'text-[#8A8278] hover:bg-[#1A1A1A]'}`}>
                {cat}
              </button>
            ))}
          </div>
